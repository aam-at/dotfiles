...
(defun aam/org-narrow-to-subtree ()
  (interactive)
...
(defun aam/org-widen-from-subtree ()
  (interactive)
...
(defun aam/org-pdfview-open (link)
...
(defun aam/org-get-active-headline-files (file)
        ...
        (org-element-map parsetree 'headline
			 (lambda (headline)
			   (when (and (= (org-element-property :level headline) 2)
			     ...
			     (let ((uuid (aam/org-id--extract-uuid
...
(defun aam/org-id--extract-uuid (input-string)
...
(defun aam/org-id--list-files (&optional directory)
    ...
    (while current-directory-list
      (cond
       ((and (equal ".org" (substring (car (car current-directory-list)) -4))
             (equal (aam/org-id--in-file (car (car current-directory-list))) t))
        (setq org-files-list
...
(defun aam/org-id-update-all ()
  "Update id locations for all files in org-directory"
  (interactive)
  (org-id-update-id-locations (aam/org-id--list-files org-directory)))

(defun aam/org-id-open--support-search (fn link &optional arg)
...
(advice-add 'org-id-open :around #'aam/org-id-open--support-search)
(advice-add 'org-roam-id-open :around #'aam/org-id-open--support-search)


;; Customization for org-roam
(defun aam/org-roam--title-to-slug (title)
...
(defun aam/org-roam-get-filepath-for-title (&optional path template)
  ...
  (setq aam/org-capture-title (read-string "Title: "))
  (let ((aam/org-capture--slug
         (read-string "Slug: " (aam/org-roam--title-to-slug aam/org-capture-title))))
    (expand-file-name
     (if (functionp template)
         (funcall template aam/org-capture--slug)
       (format template aam/org-capture--slug))
     path)))

(defun aam/org-roam-get-filepath-with-date (slug)
  "Return the filepath with datetime prefix"
  (let ((date (format-time-string "%Y%m%d-%H%M")))
    (s-lex-format "${date}_${slug}.org")))

(defun aam/org-roam-get-property-from-link (property)
...
(defun aam/org-roam-get-entry-prop (property)
  ...
  (let (entry-property)
    ...
    (unless entry-property
      (setq entry-property (aam/org-roam-get-property-from-link property)))
    entry-property))

(defun aam/org-vulpea-memo-refresh ()
  (interactive)
  (memoize-restore #'vulpea-db-query)
  (memoize         #'vulpea-db-query))

(defun aam/org-roam-force-db-sync()
  (interactive)
...
(defun aam/org-convert-org-id-link-to-file-link ()
...
(defun aam/org-transclusion-add-org-id--tofile (link &rest args)
...
(with-eval-after-load 'org-transclusion
  (add-to-list 'org-transclusion-add-functions
               #'aam/org-transclusion-add-org-id--tofile)
...
(defun aam/org-remark-notes-file ()
  (interactive)
...
(defun aam/org-citations--get-title-from-heading ()
  (let* ((heading (substring-no-properties (org-get-heading t t)))
         (title (aam/org-citations--trim-year-prefix
...
(defun aam/org-get-citations-from-link ()
  (string-to-number (aam/org-roam-get-property-from-link "CITATION_COUNT")))

(defun aam/org-set-year-from-link ()
  ...
  (let ((year (string-trim
               (aam/org-roam-get-property-from-link "YEAR"))))
    (when year
      (org-entry-put (point) "YEAR" year)
      (message "YEAR property set to %s" year))))

(defun aam/org-set-citation-from-link ()
  ...
  (let ((citation-count (string-trim
                         (aam/org-roam-get-property-from-link "CITATION_COUNT")))
        (citation-date (string-trim
                        (aam/org-roam-get-property-from-link "CITATION_LAST_UPDATED"))))
    (when citation-count
...
(defun aam/org-sort-entries-by-year ()
  "Sort Org-mode entries by the YEAR property."
  (interactive)
  (org-sort-entries nil ?F
                    (lambda ()
                      (let* ((year (aam/org-roam-get-entry-prop "YEAR")))
                        (if year
                            (string-to-number year)
                          -1)))))

(defun aam/org-sort-entries-by-citations ()
  "Sort Org-mode entries by the CITATION_COUNT property."
  (interactive)
  (org-sort-entries nil ?F
                    (lambda ()
                      (let* ((citations (aam/org-roam-get-entry-prop "CITATION_COUNT")))
                        (if citations
                            (string-to-number citations)
                          -1)))))

(defun aam/org-sort-entries-by-impact ()
...
(defun aam/org-remove-all-overlays ()
...
(defun aam/org-filter-entries-by-year (year)
  ...
  (let ((property "YEAR")
    ...
    (org-map-entries
     (lambda ()
       (let ((entry-prop (aam/org-roam-get-entry-prop property))
...
(defun aam/org-filter-entries-by-citations (citations)
...

;;  Misc functions
(defun aam/org-json-get-list (attribute message)
...
(defun aam/org-convert-json-to-review (message-string)
  ...
  (let* ((json-object-type 'alist)
    ...
    (let ((related-work-list (aam/org-json-get-list 'Related_work message))) ;; Convert vector to list
    ...
    (let ((count 1)
          (contributions-list (aam/org-json-get-list 'Contributions message)))
