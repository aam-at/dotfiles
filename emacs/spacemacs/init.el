...
This function should only modify configuration layer settings."
  (setq-default
   ...
   (append `(
             ...
             syntax-checking
             (tabs :variables tabs-auto-hide nil)
             ...
             (tree-sitter :variables
                          spacemacs-tree-sitter-hl-black-list '(org-mode latex-mode)
             ...
             (c-c++ :variables
                    ...
                    c-c++-backend ,(and (aam/lsp-client-p) 'lsp-ccls)
             ...
             emacs-lisp
             (python :variables
                     python-backend ,(and (aam/lsp-client-p) 'lsp)
             ...
             (shell-scripts :variables
                            shell-scripts-backend ,(and (aam/lsp-client-p) 'lsp)
                            shell-scripts-format-on-save t)
             (yaml :variables
                   yaml-enable-lsp ,(aam/lsp-client-p))
             ;; ----------------------------------------------------------------
             ;; Programming tools
             ;; ----------------------------------------------------------------
             dap
             ...
             ;; ----------------------------------------------------------------
             (bibtex :variables bibtex-enable-ebib-support t)
             deft
             markdown
             (latex :variables
                    latex-backend ,(and (aam/lsp-client-p) 'lsp)
             ...
             (org :variables
                  org-contacts-files (list "~/Dropbox/Org/areas/relationships.org")
                  org-enable-appear-support t
                  org-enable-bootstrap-support t
                  org-enable-brain-support t
             ...
             (org-extras :variables
                         org-enable-delve t)
             org-roam-extras
             latex-extras
             cpp-extras)

           (if (aam/eglot-client-p)
               '(eglot)
             '((lsp :variables
                    lsp-lens-enable t
...
It should only modify the values of Spacemacs settings."
  ...
  (setq-default
   ...
   dotspacemacs-themes aam/theme-variants
   ...
   dotspacemacs-default-font (aam/default-font)
...
If you are unsure, try setting them in `dotspacemacs/user-config' first."
  ...
  (add-to-list 'load-path (expand-file-name "funcs" dotspacemacs-directory))

  ;; Emacs 31.1 renamed these macros.  Suppress compatibility warnings from
  ;; third-party packages until their maintainers release updated versions.
  ...
  (if (native-comp-available-p)
    ...
    (warn "Native complation is *not* available"))
  (message "Native JSON is available")
  ;; increase lisp nesting
  (setq max-lisp-eval-depth 10000)
  ;; FIX: inhibit x resources to avoid issues with X11 for emacs 30.0.92
  (setq inhibit-x-resources t)
  ;; open true file when opening any new file
  (setq find-file-visit-truename t)
  ;; Configure shared Org and bibliography paths before layer initialization.
  (require 'aam-core)
...
before packages are loaded."
  ;; auto load configs
  (autoload 'aam/ai-setup "config-ai" "Setup AI configurations" t)
  (autoload 'aam/bibtex-setup "config-bibtex" "Setup bibtex configurations" t)
  (autoload 'aam/common-setup "config-common" "Setup common configurations" t)
  (autoload 'aam/lsp-setup "config-lsp" "Setup LSP configurations" t)
  (autoload 'aam/org-setup "config-org" "Setup org-mode configurations" t)
  ...
  (aam/common-setup)
  (if (aam/lsp-client-p)
      (with-eval-after-load 'lsp-mode (aam/lsp-setup)))
  (condition-case err
      (aam/secure-setup)
    (error
     (message "please decrypt config using git-crypt" err)))
  (aam/ai-setup)
  (aam/org-setup)
  (aam/python-setup)
  (aam/tex-setup)
