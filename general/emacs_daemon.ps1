...
$env:SERVER_HOME="$env:HOME\.emacs.d\server"
...
Set-Content -Path $env:SERVER_HOME\StartupPSLog.txt -Value (([datetime]::Now.ToString("HH:mm:ss.fff yyyy-MM-dd")) + (" Script started"))
...
if(Test-Path $env:SERVER_HOME\){
    # server directory found, remove artefacts
	Get-ChildItem -Path $env:SERVER_HOME -File | Remove-Item -Force
	Add-Content -Path $env:SERVER_HOME\StartupPSLog.txt -Value (([datetime]::Now.ToString("HH:mm:ss.fff yyyy-MM-dd")) + (" Script removed old server"))
} else {
    # new server, go
	Add-Content -Path $env:SERVER_HOME\StartupPSLog.txt -Value  (([datetime]::Now.ToString("HH:mm:ss.fff yyyy-MM-dd")) + (" Old server not found "))
}

Add-Content -Path $env:SERVER_HOME\StartupPSLog.txt -Value (([datetime]::Now.ToString("HH:mm:ss.fff yyyy-MM-dd")) + (" Starting server"))
#
# Do it
#
...
Add-Content -Path $env:SERVER_HOME\StartupPSLog.txt -Value (([datetime]::Now.ToString("HH:mm:ss.fff yyyy-MM-dd")) + (" New emacs daemon has Process Id " + ((get-process "emacs").Id)))
Add-Content -Path $env:SERVER_HOME\StartupPSLog.txt -Value (([datetime]::Now.ToString("HH:mm:ss.fff yyyy-MM-dd")) + (" Daemon started at " + ((get-process "emacs").StartTime)))
